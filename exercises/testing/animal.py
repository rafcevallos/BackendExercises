class Animal:
    def __init__(self, name = None, species = None):
        #these are properties of Animal - applicable to all
        self.name = name
    ...
    def walk(self):
        ...
        #isinstance is checking if legs is an integer
        if isinstance(self.legs, int) and self.legs > 0:
            self.speed = self.speed + (0.1 * self.legs)
        else:
            #Value Error is a way for you to control the error message which can help you determine where your error is in the code
    ...
    def __str__(self):
        # %s is one of MANY ways to format a string
        return "%s is a %s" % (self.name, self.species)


class Dog(Animal):
    def __init__(self, name):
        Animal.__init__(self, name, "Caninae")
